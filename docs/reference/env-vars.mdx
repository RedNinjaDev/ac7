...
- `OPENAI_API_KEY` — for standard OpenAI use, codex's auth comes from
  `~/.codex/auth.json`, which the runner symlinks into the ephemeral
  `CODEX_HOME`. We don't read or set this ourselves in that case.
  **Exception:** when `--model-provider` is set (local provider), the
  runner injects `OPENAI_API_KEY=local` into the codex child env if
  the variable is not already present in the operator's environment.
  Most local servers accept any non-empty value. If your server uses
  a different key name, pass it via `--wire-api` / `--model-provider`
  and set the env var in your shell before running `ac7 codex`.
