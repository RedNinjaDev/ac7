...
| `--model-provider <name>` | string | — | Short identifier for a local/custom provider (e.g. `qwen`). Must be paired with `--base-url`. |
| `--base-url <url>` | string | — | Base URL for the local OpenAI-compatible endpoint (e.g. `http://localhost:8000/v1`). Required with `--model-provider`. |
| `--wire-api <protocol>` | `chat` \| `responses` | `chat` | Wire protocol for the local provider. `chat` selects `/v1/chat/completions`; `responses` selects the Responses API. |
...
If you have a local LLM server that exposes an OpenAI-compatible API
(llama.cpp `llama-server`, vLLM, Ollama, etc.) you can point the
codex runner at it instead of OpenAI. Pass `--model-provider` and
`--base-url` together:

```bash
# llama.cpp server on port 8000 (e.g. Qwen-MoE via llama-server)
ac7 codex \
  --model-provider qwen \
...
When `--model-provider` is set:

- The ephemeral `config.toml` gets a `[model_providers.<name>]` block
  and `model_provider = "<name>"` so codex routes to your endpoint.
- `~/.codex/auth.json` is still symlinked if it exists, but **its
  absence is not an error** — local servers don't require OpenAI
  credentials.
- If the env key codex uses for the API credential (`OPENAI_API_KEY`
  by default, or the `--wire-api`-matched key for your provider) is
  not set, the runner injects `local` as a placeholder. Most local
  servers accept any non-empty value.
- Trace capture still works at the transport level. Individual
  exchanges land as `opaque_http` for now; a typed OpenAI parser is
  a planned follow-up.

**`--wire-api`** selects the HTTP endpoint shape:
...
Most local LLM servers expose the `chat` wire API. Use `responses`
only if your server explicitly supports the newer shape.
...
# Use a local llama-server (no OpenAI account needed)
ac7 codex --model-provider qwen --base-url http://localhost:8000/v1 --model qwen3-coder-30b
```
